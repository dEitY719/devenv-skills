...
Only when `run` resolves to a **run script** and `build` is a real step:
the script serves what `build` produced. Then `run: build serve` has no
recipe of its own and `serve` carries the script call. `mise run`, JS
`start`/`dev`, `go run` and `cargo run` build for themselves, so `run` calls
them directly and no `serve` is generated.
...
|---|---|---|
| `setup` | Python or JS detected, or `[tasks.setup]` | `mise run setup`; else `uv sync`, or `.venv` creation (`python<ver>` from `.python-version`, fallback `python3`) + `pip install -r <reqs>` / `-e .`; then `<runner> install` per JS app |
...
| `stop` | a server (`run` via script, JS or mise) **and** a port (detected or `--port`) | `fuser $(PORT)/tcp` then `fuser -k -TERM $(PORT)/tcp`. Never `pkill`/`killall`: a name match also hits sibling processes (brokerdesk's scheduler inherits the web server's env and name) |
| `status` | same as `stop` | port listening or not; build output present or not |
| `logs` | a log path found in the run script | `tail -f $(LOG)` |
| `test` | `[tasks.test]`, or any of pytest / JS `test` / go / cargo | `mise run test`; else every detected runner in turn: `$(PY) -m pytest` (or `uv run pytest`), JS `test` per app, `go test ./...`, `cargo test` |
...
| `lint` | `[tasks.lint]`, JS `lint`, or ruff | `mise run lint`; else JS `lint` + `ruff check .` |
| `fmt` | `[tasks.fmt]`, JS `format`/`fmt`, or ruff | `mise run fmt`; else JS script + `ruff format .` |
